...
const h5ErrorTip = "注: 在h5环境,使用HBuilderX直接点击vue文件切换和预览mescroll页面,可能导致分页失效,属于编辑器的问题,待官方修复,建议刷新页面,h5真机正常"
		...
		setTimeout(function() {
			try {
				...
				if(!pageNum){
					...
					console.log("apiWeiboList: page.num=" + pageNum + ", page.size=" + pageSize + ", curPageData.length=" + list.length);
				}
				//模拟接口请求成功
				resolute(list);
			} catch (e) {
	...
	return new Promise((resolute, reject)=>{
		...
		setTimeout(function() {
			try {
				...
				console.log("apiMsgList: page.num=" + pageNum + ", page.size=" + pageSize + ", curPageData.length=" + list.length);
				//模拟接口请求成功
				resolute(list);
			} catch (e) {
				//模拟接口请求失败
