...
export default class MeScroll {
    constructor(mescrollId, options) {
        this.version = '1.4.2.1'; // mescroll版本号
        this.isScrollBody = (!mescrollId || mescrollId === 'body'); // 滑动区域是否为body
        this.scrollDom = this.isScrollBody ? document.body : this.getDomById(mescrollId); // MeScroll的滑动区域
        ...
        // 自动加载
        setTimeout(() => { // 待主线程执行完毕再执行,避免new MeScroll未初始化,在回调获取不到mescroll的实例
            // 自动触发下拉刷新 (只有配置了down的callback才自动触发下拉刷新)
            if (this.optDown.use && this.optDown.auto && hasDownCallback) {
            ...
            // 自动触发上拉加载
            this.optUp.use && this.optUp.auto && !this.isUpAutoLoad && this.triggerUpScroll();
    ...
    /* 结束上拉加载 */
    endUpScroll(isShowNoMore, displayAble) {
    ...
    /* 联网回调成功,结束下拉刷新和上拉加载
       ...
       * systime: 服务器时间 (可空)
       */
    endByPage(dataSize, totalPage, systime) {
        var hasNext;
    ...
    /* 联网回调成功,结束下拉刷新和上拉加载
       ...
       * systime: 服务器时间 (可空)
       */
    endBySize(dataSize, totalSize, systime) {
        var hasNext;
    ...
    setTopBtnFadeDuration(fadeDuration) {
        if (this.toTopBtn) {
